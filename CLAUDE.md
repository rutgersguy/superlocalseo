...
- **BrightLocal Management API** — Citation Builder uses `https://api.brightlocal.com/manage/v1/citation-builder` with `x-api-key`. Access verified 2026-09-15; 30 credits available before submissions. Both admin and customer paid confirmations MUST use `citation_submission.service.ts`. Pro, verified non-zero payment, curated relevant directories, one initial campaign capped at 15 credits; no trial spending or included extras. See `docs/LISTING_SUBMISSION_POLICY.md`.
...
- **BrightLocal APIs:** Citation Builder Management v1 uses `api.brightlocal.com/manage/v1/citation-builder` and the configured `x-api-key`. Do not replace it with the retired tools.brightlocal.com integration or assume all Management endpoints share that legacy authentication.
