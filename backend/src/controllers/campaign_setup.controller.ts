...
import { setupView, verificationBinding, CAMPAIGN_TEMPLATE_VERSION } from '../services/campaign_verification';
...
export async function requestSetup(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    ok(res, setupView(row, location, client, campaigns, locations.length));
  } catch (e) { next(e); }
}
...
export async function setupStatus(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    ok(res, { locations, requests: requests.map(r => setupView(r, fullLocations.find(l => l.id === r.location_id), client, campaigns, fullLocations.length)) });
  } catch (e) { next(e); }
}
...
export async function setupQueue(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const requests = rows.map(r => {
      ...
      return { ...r, ...setupView({ ...r, location_id: location?.id, created_at: r.requestedAt, updated_at: r.updatedAt }, location,
        clients.find(c => c.id === r.clientId), campaigns.filter(c => c.client_id === r.clientId), ownLocations.length),
  ...
  try {
    ...
    const result = await db.transaction(async trx => {
      ...
      if (p.status === 'verified') {
        ...
        const client = await trx('clients').where({ id: r.client_id }).forUpdate().first();
        ...
        if (locations.length !== 1) return { code: 409, message: 'Multi-location provider mapping requires manual reconciliation before verification is supported.' };
        ...
        if (!campaign || !campaign.metrics_pulled_at) return { code: 409, message: 'Select a campaign synced to this customer first.' };
