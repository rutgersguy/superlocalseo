...
import { operatorInspectionSchema } from '../services/provider_mapping_evidence';
import { config } from '../config';
import { saveProviderMapping, mappingIdentity } from '../services/provider_mapping.service';
...
const updateSchema = z.object({ revision: z.number().int().min(0), organizationId: providerId, providerLocationId: providerId,
  googlePlaceId: z.string().regex(/^[A-Za-z0-9_-]{5,255}$/), expectedGooglePlaceId: z.string().regex(/^[A-Za-z0-9_-]{5,255}$/).nullable(), note: z.string().trim().min(5).max(2000), inspection: operatorInspectionSchema }).strict();
  ...
  try {
    ...
    const rows = await db('locations as l').join('clients as c', 'c.id', 'l.client_id')
      .select('l.id as locationId', 'l.client_id as clientId', 'c.business_name as businessName', 'l.name as locationName', 'l.city',
        'l.address', 'l.state', 'l.zip', 'l.google_place_id as googlePlaceId', 'c.emr_organization_id as legacyOrganizationId', 'c.emr_location_id as legacyProviderLocationId')
    ...
    ok(res, { locations: rows.map(r => {
      ...
      return { ...r, mapping: m ? { organizationId: m.organization_id, providerLocationId: m.provider_location_id,
        ...
        identityCurrent: r.googlePlaceId === m.google_place_id && m.evidence?.localIdentity === mappingIdentity({ id: r.locationId, client_id: r.clientId, name: r.locationName, address: r.address, city: r.city, state: r.state, zip: r.zip, google_place_id: r.googlePlaceId }),
    ...
    }), page, total, hasMore: page * 50 < total, verificationAvailable: Boolean(config.embedmyreviews.apiKey), verificationMode: 'operator_inspection', providerDashboardUrl: config.embedmyreviews.baseUrl,
      verificationUnavailableReason: config.embedmyreviews.apiKey ? null : 'Provider API credentials are not configured.' });
  } catch (e) { next(e); }
}
