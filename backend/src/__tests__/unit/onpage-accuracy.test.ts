...
describe('audit PDF literal markup', () => {
  it('escapes observed tags so a missing-title finding cannot consume the remaining report', () => {
    const html = renderAuditReportHtml({location_name:'Example <script>alert(1)</script>',on_page_details:['No <title> tag found — add a title']});
    expect(html).toContain('No &lt;title&gt; tag found');
    expect(html).not.toContain('No <title> tag found');
    expect(html).not.toContain('<script>alert(1)</script>');
    expect(html).toContain('Generated by SuperLocalSEO');
  });
});
