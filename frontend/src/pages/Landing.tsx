...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border-b border-gray-200 last:border-0">
      <button
        className="w-full flex items-center justify-between py-5 text-left text-gray-900 font-medium text-sm hover:text-brand-500 transition-colors"
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
      >
        {q}
        <ChevronDown
          size={18}
          className={`flex-shrink-0 ml-4 text-gray-400 transition-transform duration-200 ${open ? 'rotate-180' : ''}`}
        />
      </button>
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Nav */}
      <nav className="border-b border-gray-100 px-6 py-4" aria-label="Main navigation">
        ...
        {/* Mobile menu */}
        {mobileMenuOpen && (
          <div className="sm:hidden pt-4 pb-2 space-y-2 border-t border-gray-100 mt-4">
      ...
      <main id="main-content">
        ...
        {/* Pricing */}
        <section className="bg-gray-50 py-16 sm:py-20 px-6" aria-labelledby="pricing-heading">
          <div className="max-w-4xl mx-auto text-center mb-12">
            <h2 id="pricing-heading" className="text-3xl font-bold text-gray-900 mb-4">Simple, location-based pricing</h2>
