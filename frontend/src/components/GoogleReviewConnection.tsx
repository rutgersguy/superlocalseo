...
export default function GoogleReviewConnection() {
  ...
  const selectedId = locationId || (locations.length === 1 ? locations[0].id : '');
  const locationQuery = selectedId ? '?locationId=' + encodeURIComponent(selectedId) : '';
  const needsSelection = locations.length > 1 && !selectedId;
  const { data, error: statusError, isLoading, mutate } = useSWR<{ data: ConnectionState }>(needsSelection ? null : '/integrations/emr/google/connect-link' + locationQuery, fetcher, { dedupingInterval: 30000, shouldRetryOnError: false });
  ...
  async function importReviews() {
    ...
    try {
      const result = await apiFetch<{ success: boolean; error?: { message: string } }>('/integrations/emr/google/sync', { method: 'POST', body: JSON.stringify({ locationId: selectedId || undefined }) });
  ...
  async function connect() {
    ...
    try {
      const result = await apiFetch<{ success: boolean; data?: { connectUrl: string }; error?: { message: string } }>('/integrations/emr/google/connect-link', { method: 'POST', body: JSON.stringify({ locationId: selectedId || undefined }) });
  ...
  useEffect(() => { setReadyUrl(null); setError(''); setSyncMessage(''); setWaitingSince(null); autoImport.current = null; }, [selectedId]);
  ...
  return <section aria-label="Google review connection" className="rounded-xl border border-slate-200 bg-white p-5 space-y-3">
    ...
    {locations.length > 1 && <label className="block text-sm">Business location<select value={locationId} onChange={e => setLocationId(e.target.value)} disabled={working || syncing} className="block w-full rounded-lg border-slate-300"><option value="">Select a location</option>{locations.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}</select></label>}
    <p className="text-sm font-medium text-slate-800" role="status">{needsSelection ? 'Select a business location' : isLoading ? 'Checking connection…' : statusError ? 'Unable to check connection status' : title}</p>
    ...
    <div className="flex flex-wrap items-center gap-4">
      <button type="button" disabled={needsSelection || working || state?.phase === 'needs_attention'} onClick={() => void connect()} className="rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{working ? 'Preparing connection…' : state?.profileSelected ? 'Reconnect Google' : 'Connect Google'}</button>
      {state?.profileSelected && <button type="button" disabled={needsSelection || syncing} onClick={() => void importReviews()} className="text-sm text-slate-700 underline">{syncing ? 'Requesting import…' : 'Refresh reviews'}</button>}
