...
interface SetupRow { mappingReady?: boolean; status: string; revision: number; googlePlaceId: string | null; locationCount: number; verifiedAt: string | null; events: Array<{ revision: number; status: string; note: string; actorId: string | null; createdAt: string; verification: { campaignId: string; googlePlaceId: string; templateVersion: string; verifiedAt: string } | null }>;  id: string; requestedAt: string; businessName: string; locationName: string; city: string | null; organizationId: number | null; providerLocationId: number | null; campaigns: Array<{ name: string; providerId: string; lastSyncAt: string | null }> }
  ...
  return <details className="text-sm"><summary className="cursor-pointer font-medium">Update setup / record verification</summary>
    <form onSubmit={save} className="mt-3 space-y-3">
      ...
      {status === 'verified' && <>
        ...
        {(row.locationCount !== 1 && !row.mappingReady) && <p role="alert">This branch needs a current provider mapping inspection before campaign verification. Record a blocked status and the required reconciliation.</p>}
      ...
      <button disabled={busy || (status === 'verified' && ((row.locationCount !== 1 && !row.mappingReady) || !row.googlePlaceId))} className="rounded-lg bg-slate-800 px-4 py-2 text-white disabled:opacity-50">{busy ? 'Saving…' : 'Save setup record'}</button>
