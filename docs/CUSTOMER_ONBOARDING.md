...
5. Open Review requests → Campaign setup, select an owned location and request assisted setup. This persists one request per client/location and sends no invitations. The customer sees requested/in-progress/blocked/configuration-verified status after an operator records checks; configuration verification is not delivery confirmation.
...
5. Repeat destination verification on a second designated business before treating a template as reusable. Record the required checks and internal note using [Admin verification](campaign-setup-verification.md). Multi-location verification remains blocked until explicit provider mapping is supported.
...
- #207: external acceptance and broader recovery beyond the implemented setup verification/audit workflow.
