...
The parallel CI suite exposed a mapping-save PostgreSQL deadlock against concurrent account activity. Mapping saves now retry only rolled-back SQL deadlocks (maximum three attempts), re-running revision/identity/freshness checks without repeating the provider inspection. Other failures are not retried; exhausted deadlocks return a recoverable 409. Regression tests cover retry selection and bounds.
