...
Individual map-provider failures now leave explicitly unverified points while the remaining points are collected. Unverified points are excluded from coverage percentages. An operator may call `resumeInitialMap(clientId, locationId, reportId)` after inspecting a failed three-by-three report; it preserves saved points and only requests missing coordinates. It requires matching ownership, keyword and current map center and cannot rerun a completed report.
