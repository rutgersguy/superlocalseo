...
## Legacy vendor entry points

Versioned vendor header snippet: `integrations/embedmyreviews/report-routing.html`. Install through the supported White-Label → Configuration → Advanced → Header Scripts UI. The existing header and body script fields were empty when inspected. The snippet is restricted to app.superlocalseo.com and the observed public `/intel-request` and `/business-report/{token}` route shapes. It does not intercept agency tools, login, OAuth/connect, review collection, or customer dashboards. Query strings and opaque report IDs are not forwarded to the native site. Old report links lead to `/audit?legacy=1`, which explains that a newly requested report is a new snapshot; no vendor report data is copied, deleted, or passed off as current source data. Native homepage traffic already uses `/audit`.

Validate anonymously after saving: old form → native builder, old report → migration notice, agency settings and reviews unaffected. Custom header execution depends on vendor page support and JavaScript. If a public template omits the header snippet, record that limitation rather than claiming it redirects. Rollback: remove only this marked snippet in the same vendor setting. Keep the native migration notice and all saved reports. Test routing locally with `node scripts/test-emr-report-routing.cjs`.

## Deployed routing verification — PR #215

Main `58fdc82b` is deployed. The versioned header snippet was saved through EMR’s supported settings UI. Fresh anonymous browser checks confirmed `/intel-request` reaches the native builder and all three existing shared vendor report links (Light Hawk Studios, In Da Cut Barber Lounge, Tulsa Air Specialists Inc) reach the migration notice. Login remained available; no report was generated by those visits. This is client-side routing and leaves vendor records intact. See [current release status](RELEASE_STATUS.md) for the remaining launch work.
